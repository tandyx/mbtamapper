...
def getvehicles(vehicle_type=2):
    ...
    if req.ok:
        ...
        vehicles["trip_short_name"] = mbta_response["trip"].apply(lambda x: x["name"])
        vehicles["headsign"] = mbta_response["trip"].apply(lambda x: x["headsign"])
        vehicles["stop"] = mbta_response["stop"].apply(lambda x: x["id"])
        vehicles["stop_name"] = mbta_response["stop"].apply(lambda x: x["name"])
